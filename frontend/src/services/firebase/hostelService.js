...
// ─── Rooms ────────────────────────────────────────────────────────────────────
export async function listRooms({ block, type } = {}) {
  if (!isFirebaseConfigured || !db) return [];
  return safe(async () => {
    // Fetch all tenant rooms without orderBy to avoid composite index requirement; filter+sort client-side
    const snap = await getDocs(tenantQ('hostel_rooms'));
    let rows = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
    if (block) rows = rows.filter((r) => r.block === block);
    if (type)  rows = rows.filter((r) => r.type  === type);
    return rows.sort((a, b) => (a.block || '').localeCompare(b.block || '') || (a.number || '').localeCompare(b.number || ''));
  }, []);
}
...
// ─── Allocations ──────────────────────────────────────────────────────────────
export async function listHostelAllocations({ roomId, studentId } = {}) {
  if (!isFirebaseConfigured || !db) return [];
  return safe(async () => {
