...
export default function SubjectsTopicsCRUD() {
  ...
  const [subjForm, setSubjForm] = useState({ name: '', code: '', className: '10th', section: '', teacherId: '', type: 'Core' });
  ...
  const saveSubject = async () => {
    ...
    if (subjModal === 'add') { 
      ...
      await updateSubject(subjModal, subjForm); 
      setSubjects((s) => s.map((x) => x.id === subjModal ? { ...x, ...subjForm } : x));
  ...
  const delSubject = async (s) => {
    ...
    setSubjects((l) => l.filter((x) => x.id !== s.id));
    if (active === s.id) setActive(subjects[0]?.id || null);
  ...
  const saveTopic = async () => {
    ...
    if (topicModal === 'add') {
      const row = await addTopic(payload);
      if (row) setTopics((t) => [...t, row]);
    } else {
      await updateTopic(topicModal, payload);
      setTopics((t) => t.map((x) => x.id === topicModal ? { ...x, ...payload } : x));
    }
    setSaving(false);
  ...
  const updateTopicStatus = async (id, status) => { 
    ...
    setTopics((t) => t.map((x) => x.id === id ? { ...x, status } : x));
  };
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm grid place-items-center p-4">
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-card rounded-[2rem] p-6 w-full max-w-md border border-border">
            ...
            <div className="space-y-3">
              ...
              <button onClick={saveTopic} disabled={saving} className="w-full h-11 rounded-2xl bg-primary text-primary-foreground label-eyebrow disabled:opacity-60" data-testid="st-topic-save">{saving ? 'Saving...' : 'Save Topic'}</button>
            </div>
          </motion.div>
        </div>
      )}
